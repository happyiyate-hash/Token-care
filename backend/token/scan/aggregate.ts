...
export async function aggregateTokenProviders(input: TokenScanInput): Promise<AggregatedTokenData> {
  const results = await Promise.all([
    fetchDexScreener(input),
    fetchGeckoTerminal(input),
    fetchCoinGecko(input),
    fetchCoinMarketCap(input),
    fetchBirdeye(input),
    fetchDexTools(input),
  ]);
  ...
  return {
    token: {
      name: pick(results, (d) => d.name),
      symbol: pick(results, (d) => d.symbol),
      decimals: pick(results, (d) => d.decimals),
      total_supply: pick(results, (d) => d.total_supply),
      logo_url: logoProvider?.data?.logo_url || null,
      logo_source: logoProvider?.provider || null,
    },
    market: {
      ...
      market_cap: numericConsensus(results, (d) => d.market_cap),
      fdv: numericConsensus(results, (d) => d.fdv),
      pairs_count: pick(dexResults, (d) => d.pairs_count),
      buys_24h: pick(dexResults, (d) => d.buys_24h),
      sells_24h: pick(dexResults, (d) => d.sells_24h),
      dexes: [...new Set(dexResults.flatMap((r) => r.data?.dexes || []))],
