...
/**
 * Backwards-compatible export used by Sidebar and other existing consumers.
 * Every EVM entry resolves to its own Trust Wallet blockchain logo; there is
 * deliberately no Ethereum fallback for unknown chains.
 */
        ...
        <div className="p-3 overflow-y-auto space-y-1.5 flex-1 no-scrollbar bg-[#06080E]">
          {filteredChains.length === 0 ? <div className="p-6 text-center text-zinc-500 text-xs font-mono">No blockchain found matching "{searchTerm}"</div> : <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
            {filteredChains.map((chain, index) => {
              ...
              return <button key={uniqueKey} type="button" disabled={!chain.supported} onClick={() => { if (!chain.supported) return; onSelectChain(chain.id); onClose(); }} className={`text-left px-3 py-2 rounded-xl border transition-all flex items-center justify-between group ${!chain.supported ? 'bg-[#090B11] border-zinc-900/80 opacity-55 cursor-not-allowed' : isSelected ? 'bg-emerald-500/15 border-emerald-500/60 shadow-md ring-1 ring-emerald-500/30 cursor-pointer' : 'bg-[#0B0E17] border-zinc-800/80 hover:bg-zinc-900 hover:border-zinc-700 cursor-pointer'}`}>
                <div className="flex items-center space-x-2.5 min-w-0">
                  <div className="w-8 h-8 rounded-full bg-zinc-900 border border-zinc-800 p-0.5 flex items-center justify-center shrink-0 overflow-hidden">
                    {logoUrl && !hasError ? <img src={logoUrl} alt={chain.name} className="w-full h-full object-cover rounded-full" onError={() => handleImageError(uniqueKey)} /> : <div className="w-full h-full rounded-full flex items-center justify-center font-mono text-[9px] font-bold text-white bg-zinc-700">{chain.symbol.slice(0, 3)}</div>}
                  </div>
                  <div className="min-w-0"><div className="flex items-center space-x-1"><span className="text-xs font-bold text-white truncate group-hover:text-emerald-400 transition-colors">{chain.name}</span><span className="text-[10px] text-zinc-400 font-mono">({chain.symbol})</span></div><div className="text-[10px] text-zinc-500 font-mono truncate">{chain.chainId ? `Chain ID: ${chain.chainId}` : chain.tokenStandard}</div></div>
                </div>
