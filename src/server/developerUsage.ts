...
export async function validateAndConsumeDeveloperQuota(options: { apiKey?: string | null; endpoint: string; method?: string; action?: string; authHeader?: string | null }): Promise<DeveloperUsageValidationResult> {
  ...
  try {
    ...
    if (project.is_active === false) return { hasKey: true, allowed: false, statusCode: 403, error: { code: 'PROJECT_PAUSED', message: 'This developer project is currently paused.' }, project };
...
export async function finalizeDeveloperRequestLog(options: { requestId?: string; statusCode: number; latencyMs: number; errorCode?: string | null }): Promise<void> {
  if (!options.requestId) return;
  try { await getServerSupabase().from('developer_request_logs').update({ status_code: options.statusCode, latency_ms: options.latencyMs, error_code: options.errorCode || null, completed_at: new Date().toISOString() }).eq('request_id', options.requestId); }
  catch (e) { console.warn('[Developer Usage Service] Finalize log note:', e); }
}
